...
public abstract class ZombieMainGoal extends Goal {
    ...
    public static EntityPredicate conditionsEntity = (new EntityPredicate()).range(75d).selector(
            (LivingEntity::isAlive));
    public final int RANDOM_NUM = RandomUtils.nextInt(-100, 100);
    ...
    public ZombieMainGoal(ZombieEntity zombie) {

        this.zombie = zombie;
        this.SPEED_FIX = EntityDataManager.getOrCreate(zombie, "speed_fix", RandomUtils.nextDouble(-0.2d, 0));
    ...
    public void tick() {
        ...
        if ((this.zombie.level.getGameTime() % 5L == RandomUtils.nextInt(0, 5))) {
            if ((this.zombieRandomWalkGoal.isUsable) && (!this.zombieRandomWalkGoal.goal.isWalking())) {
                ZombieEntity zombie_ = this.zombie.level.getNearestEntity(ZombieEntity.class, conditionsEntity, this.zombie,
                ...
                if (zombie_ != null) {
                    if (RandomUtils.nextBoolean(0.1d)
                        && zombie_.getEyePosition(0).distanceTo(this.zombie.getEyePosition(0)) <= 2d
    ...
    public void callToAttack(LivingEntity target) {
        ...
        this.zombie.level.getNearestEntity(ZombieEntity.class,
                (new EntityPredicate()).range(75d).selector((entity) -> {
                    if (this.zombie.getSensing().canSee(entity)
                        || GameProperty.TimeProperty.getGameStage() >= 0.6d) {
                        if (ZombieUtils.canBeLoaded(entity)) {
                            ZombieEntity zombie1 = (ZombieEntity) entity;
                    ...
                    return false;
                }), this.zombie, this.zombie.getX(), this.zombie.getY(), this.zombie.getZ(),
    ...
    public final boolean canPlaceBlock() {
        return this.canPlace() && Options.CAN_ZOMBIE_PLACE_BLOCK.getValue((ServerWorld) this.zombie.level);
    }

    public final boolean canBreakBlock() {
        return this.canBreak() && Options.CAN_ZOMBIE_BREAK_BLOCK.getValue((ServerWorld) this.zombie.level);
    }
