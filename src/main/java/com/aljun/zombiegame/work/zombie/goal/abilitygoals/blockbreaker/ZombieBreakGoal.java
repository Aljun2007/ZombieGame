...
public class ZombieBreakGoal extends AbstractZombieAbilityGoal {
    ...
    public ZombieBreakGoal(ZombieMainGoal mainGoal, ZombieEntity zombie) {
        super(mainGoal, zombie);
        this.level = (ServerWorld) zombie.level;
    }

    private static float getBreakProgress(BlockState state, ZombieEntity zombie, BlockPos pos) {

        ItemStack stack = zombie.getItemBySlot(EquipmentSlotType.MAINHAND);
    ...
    public static float getDigSpeed(BlockState state, @Nullable BlockPos pos, ItemStack stack, ZombieEntity zombie) {
        ...
        if (EffectUtils.hasDigSpeed(zombie)) {
            f *= 1.0F + (float) (EffectUtils.getDigSpeedAmplification(zombie) + 1) * 0.2F;
        }

        if (zombie.hasEffect(Effects.DIG_SLOWDOWN)) {
            float f1;
            switch (Objects.requireNonNull(zombie.getEffect(Effects.DIG_SLOWDOWN)).getAmplifier()) {
                case 0:
                    f1 = 0.3F;
                    break;
                case 1:
        ...
        if (zombie.isEyeInFluid(FluidTags.WATER) && !EnchantmentHelper.hasAquaAffinity(zombie)) {
            f /= 5.0F;
        }
    ...
    private static void destroyBlock(ZombieEntity zombie, BlockPos pos) {
        ItemStack stack = zombie.getItemBySlot(EquipmentSlotType.MAINHAND);
        BlockState state = zombie.level.getBlockState(pos);
        World level = zombie.level;
        zombieDestroy(level, zombie, pos, state, level.getBlockEntity(pos), stack);
        level.destroyBlock(pos, false, zombie);

    }

    public static void zombieDestroy(World level, ZombieEntity zombie, BlockPos pos, BlockState state,
                                     @Nullable TileEntity blockEntity, ItemStack stack) {
        ...
        if (checkCanContinue(this.pos)) {
            ...
            if (this.progress >= 1f) {
                ...
                if ((this.zombie.level.getGameTime() - this.startTime) % 4L == 0) {
                    ...
                    this.zombie.level.playSound(null, this.pos, soundType.getHitSound(), SoundCategory.BLOCKS,
    ...
    private boolean isPositionCorrect(BlockPos pos) {
        return this.zombie.level.getBlockState(pos).canEntityDestroy(this.zombie.level, pos, this.zombie)
                ...
                && this.zombie.getEyePosition(0).distanceTo(Tools.blockPosToVec3(pos)) <= 3d;
    }
    ...
    private boolean isStateCorrect(BlockState state) {
        ...
        if (block2 instanceof FlowingFluidBlock) {
            canBreak = false;
        } else if (block2 instanceof CommandBlockBlock || block2 instanceof StructureBlock || block2 instanceof JigsawBlock
        ) {
            canBreak = false;
        } else {
