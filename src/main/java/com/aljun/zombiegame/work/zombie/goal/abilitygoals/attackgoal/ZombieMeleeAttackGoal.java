...
public class ZombieMeleeAttackGoal extends AbstractZombieAbilityGoal {
    ...
    public ZombieMeleeAttackGoal(ZombieMainGoal mainGoal, ZombieEntity zombie) {
                ...
                if (this.ticksUntilNextPathRecalculation == 0 && (this.targetX == 0.0D
                    ...
                    if (canMove() && !this.startBuilding) {
                            ...
                            if (Math.sqrt(this.zombie.blockPosition().distSqr(targetPos)) <= 20d) {
                                if (path.getDistToTarget() >= 3d) {
                                    if (this.mainGoal.canUsePathBuilder()
                                        ...
                                        this.setBuild();
                                    } else if (GameProperty.TimeProperty.getGameStage() >= ConfigDataManager.getOrDefault(
                                            ConfigKeySets.JUMP_STAGE)
                                               && this.zombie.isOnGround()
                                               && this.zombie.getEyePosition(0).distanceTo(
                                            this.zombie.getTarget().getEyePosition(0)) <= 3d
                        ...
                        if (!this.startBuilding) {
                            ...
                            boolean a2 = this.zombie.getEyePosition(0).distanceTo(
                                    this.zombie.getTarget().getEyePosition(0)) <= 3.5d;
    ...
    private void setBuild() {
        if (!this.startBuilding) {
            Vector3d vec3 = DefaultRandomPos.getPos(this.getZombie(), 6, 4);
            if (vec3 == null) {
