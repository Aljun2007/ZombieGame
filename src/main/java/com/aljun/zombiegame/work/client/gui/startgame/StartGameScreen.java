...
public class StartGameScreen extends Screen {
    ...
    protected void init() {
        ModButton yesButton = new ModButton(this.width / 2 - 80 - 40, this.height / 2 + 30, 80, 20,
                new TranslatableComponent("gui.zombiegame.button.start"), (b) -> this.yes());
        ModButton noButton = new ModButton(this.width / 2 + 80 - 40, this.height / 2 + 30, 80, 20,
                new TranslatableComponent("gui.zombiegame.button.cancel"), (b) -> this.no());
        ModStringWidgetMod titleStr = new ModStringWidgetMod(
                new TranslatableComponent("gui.zombiegame.start_game.title"), this.font);
        ...
        this.modeButton = new ModButton(this.width / 2 - 80 - 40, this.height / 2 + 55, 80, 20,
                new TranslatableComponent("gui.zombiegame.button.start"), (b) -> this.nextMode());
        this.dayTime = new ForgeSlider(this.width / 2 - 20, this.height / 2 + 55, 120, 20,
                new TranslatableComponent("gui.zombiegame.start_game.day_time_pr"),
                new TranslatableComponent("gui.zombiegame.start_game.day_time_sr"), 10, 1000, 100, 10, 0, true);
        this.gameStage = new ForgeSlider(this.width / 2 - 20, this.height / 2 + 55, 120, 20,
                new TranslatableComponent("gui.zombiegame.start_game.game_stage_pr"),
                new TranslatableComponent("gui.zombiegame.start_game.game_stage_sr"), 0, 100, 60, 10, 0, true);
    ...
    private void yes() {
        Chatting.sendMessageLocalPlayerOnly(new TranslatableComponent("message.zombiegame.warn",
                new TranslatableComponent("message.zombiegame.startgame.starting")));
    ...
    private void no() {
        Chatting.sendMessageLocalPlayerOnly(new TranslatableComponent("message.zombiegame.warn",
                new TranslatableComponent("message.zombiegame.startgame.canceled")));
        super.onClose();
    }

    @Override
    public void onClose() {
        Chatting.sendMessageLocalPlayerOnly(new TranslatableComponent("message.zombiegame.error",
                new TranslatableComponent("message.zombiegame.common_screen.unexpected_shutdown")));
        super.onClose();
    }
    ...
    public void tick() {
        ...
        if (this.mode.equals("normal")) {
            if (this.modeButton != null) {
                this.modeButton.setMessage(new TranslatableComponent("gui.zombiegame.start_game.mode_button.normal"));
            }
            ...
            if (this.modeButton != null) {
                this.modeButton.setMessage(
                        new TranslatableComponent("gui.zombiegame.start_game.mode_button.remain").withStyle(
                                ((style) -> style.withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                                        new TranslatableComponent("message.zombiegame.info.remain"))))));
            }
            if (this.gameStage != null) {
